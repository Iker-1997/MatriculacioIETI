      ...
      $.get({
        ...
        $.getJSON({
          ...
          for (const term in response){
            $("tbody").append('<tr class="text-center" data-id="'+response[term][`id`]+'""><td class="border-2 border-mtr-dark text"><a href="">'+response[term][`name_terms`]+'</a></td><td class="hidden border-2 border-mtr-dark input"><input value="'+response[term][`name_terms`]+'" type="text" name="nameEdit" class="nameEdit"></td><td class="border-2 border-mtr-dark text">'+response[term][`description_terms`]+'</td><td class="hidden border-2 border-mtr-dark input"><input value="'+response[term][`description_terms`]+'" type="text" name="descEdit" class="descEdit"></td><td class="border-2 border-mtr-dark text">'+response[term][`start`]+'</td><td class="hidden border-2 border-mtr-dark input"><input value="'+response[term][`start`]+'" type="datetime" name="startEdit" class="startEdit bg-white"></td><td class="border-2 border-mtr-dark text">'+response[term][`end`]+'</td><td class="hidden border-2 border-mtr-dark input"><input value="'+response[term][`end`]+'" type="datetime" name="endEdit" class="endEdit bg-white"></td><td class="p-2 flex flex-col justify-center md:flex-row space-y-2 md:space-y-0 md:space-x-6"><button id="edit" onClick="editRow('+response[term][`id`]+')" class="bg-mtr-dark py-2 px-4 text-white rounded text">Edit</button><button id="save" onClick="saveChange('+response[term][`id`]+')" class="hidden bg-mtr-dark py-2 px-4 text-white rounded input">Save</button><button id="cancel" onClick="cancelChange('+response[term][`id`]+')" class="hidden bg-red-500 py-2 px-4 text-white rounded input">Cancel</button><a href="/admin/terms/delete/'+response[term][`id`]+'" class="bg-red-500 py-2 px-4 text-white rounded text">Delete</a></td></tr>');
          }
        });
      });
    }
